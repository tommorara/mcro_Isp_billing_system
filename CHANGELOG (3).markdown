...
- Fixed `ImportError: cannot import name 'PaymentPlugin' from 'plugins.base'` by adding `PaymentPlugin` class to `plugins/base.py`.
...
- Ensure `plugins` app is in `INSTALLED_APPS` in `settings.py`.